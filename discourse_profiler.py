...
class Classifier(nn.Module):
    ...
    def forward(self, reference, sentence, len_1, len_2, hidden_ctxt=None):

        character_ids = batch_to_ids(reference)
        if CUDA:
            character_ids = character_ids.cuda()
        embedding = elmo(character_ids)
        outp_ctxt = embedding['elmo_representations'][0]
        ctxt_mask_1 = embedding['mask']
        outp_1 = self.context_encoder.forward(outp_ctxt, len_1, hidden_ctxt)
        ...
        self_attention = F.tanh(self.ws1(self.drop(outp_1)))
        self_attention = self.ws2(self.drop(self_attention)).squeeze()
        self_attention = self_attention + -10000*(ctxt_mask_1 == 0).float()
        self_attention = self.softmax(self_attention)
        ref_encoding = torch.sum(outp_1*self_attention.unsqueeze(-1), dim=1)
        ...
        #print(inner_pred.size())
        inner_pred = inner_pred.squeeze(0)
